...
typedef enum UnityXRRenderTextureFlags
{
    ...
    /// This texture can be an unresolved MSAA texture.  Tile-based archictures can benefit with reduced bandwidth by accepting unresolved textures.
    ...
    /// Opt-in to always discarding depth and resolving MSAA color to improve performance on tile-based architectures at the expense of rarely-used
    /// effects which require depth resolve or MSAA color store, such as camera stacking. This only affects Vulkan.
    /// Note that this may break user content - use with care and consider giving the developer a way to turn it off.
...
///        }]
/// }],
/// Default blit for platform.
const static int kUnityXRMirrorBlitDefault = 0;
/// Left eye blit
...
const static int kUnityXRMirrorBlitNone = -6;
...
typedef enum UnityXRFrameSetupHintsChanged
{
    ...
    /// The engine has changed the znear plane state.  Check the #UnityXRFrameSetupHints::UnityXRAppSetup `zNear` field.
    kUnityXRFrameSetuphintsChangedZNear = 1 << 7,

    /// The engine has changed the zfar plane state.  Check the #UnityXRFrameSetupHints::UnityXRAppSetup `zFar` field.
    kUnityXRFrameSetuphintsChangedZFar = 1 << 8,

    /// The engine has changed the srgb state.  Check the #UnityXRFrameSetupHints::UnityXRAppSetup `sRGB` field.
    kUnityXRFrameSetuphintsChangedSRGB = 1 << 9,
