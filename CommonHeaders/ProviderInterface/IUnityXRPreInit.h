...
typedef enum UnityXRPreInitFlags
{
    ...
    /// Plugin requests an additional Vulkan graphics queue for its own rendering. Plugin should check for the additional queue before using it.
    kUnityXRPreInitFlagsRequestAdditionalVulkanGraphicsQueue = (1 << 3),

    /// Plugin requests the use of an offscreen Vulkan swapchain.
